...
$(document).on('input','#name', function (e) {
		  ...
		  if(name.charAt(0)==' '){
				 if($(this).next().hasClass('name_error')){
					$(this).next().remove();
				 }
			  $(this).after("<small class='dangererror pull-left name_error'>Space not allowed in begin</small>");
		   }else{
			   $(this).next().remove();
		   }
...
$("form").submit(function(){
	...
	if(banner_image=='')
	{
		$( "input[name='banner_image']" ).after("<small class='dangererror pull-left banner_error'>Banner Image should not be blank </small>");
		return false;
	}else{
		$( ".banner_error" ).remove();
	}
